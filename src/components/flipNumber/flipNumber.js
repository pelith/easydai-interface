...
const calculateDegrees = (rotateCounter, activeNumber) => {
  const animateDegree =
    numbers.findIndex(v => v === activeNumber) * rotateDegreePerNumber
  ...
  return {
    ...(activeNumber === 0
      ? {
          rotateCounter:
            rotateCounter > resetRouteCounter ? 0 : rotateCounter + 1,
        }
      : null),
...
export default class FlipNumber extends React.Component<Props, State> {
  ...
  componentDidMount() {
    this.updateNumberTimeout = setTimeout(
      () => this.updateNumber(),
  ...
  updateNumber = () => {
    this.setState(({ rotateCounter }) =>
      calculateDegrees(rotateCounter, this.props.activeNumber),
    )
  }
              ...
              {numbers.map((n, i) => (
                <span
                  style={{
                    ...
                    transform: `rotateX(${rotateDegreePerNumber *
                      i}deg) translateZ(${translateZ}px)`,
                    ...numberStyle,
                  }}
